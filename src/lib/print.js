...
.empty td{height:18px}.biko{font-weight:bold;letter-spacing:6px;vertical-align:top;width:50px}`;
  ...
  if (type === "invoice") {
    ...
    let _pdfEquipSum = 0;
    (()=>{const _bNo=dn=>(dn||"").replace(/E\d+.*$/,"");const chainEndMap={};g.items.forEach(r=>{const bk=_bNo(r.deliveryNo);if(!bk)return;const re=r.returnDate||r.endDate||"";if(!chainEndMap[bk]||re>chainEndMap[bk])chainEndMap[bk]=re;});const gKey=r=>{const bk=_bNo(r.deliveryNo);return(bk&&chainEndMap[bk])||(r.returnDate||r.endDate||"");};const chainStartMap={};g.items.forEach(r=>{const bk=_bNo(r.deliveryNo);if(!bk)return;const st=r.startDate||"";if(!chainStartMap[bk]||st<chainStartMap[bk])chainStartMap[bk]=st;});const sKey=r=>{const bk=_bNo(r.deliveryNo);return(bk&&chainStartMap[bk])||(r.startDate||"");};const _tier=r=>r.billingType==="monthly"?0:((r.isProvisionalClose||r.endDateOpen)?2:1);const _sorted=[...g.items].sort((a,b)=>{const ta=_tier(a),tb=_tier(b);if(ta!==tb)return ta-tb;if(ta===0){const kc0=gKey(a).localeCompare(gKey(b));if(kc0!==0)return kc0;return(a.isExtension?1:0)-(b.isExtension?1:0);}const ec=gKey(a).localeCompare(gKey(b));if(ec!==0)return ec;const sc=sKey(a).localeCompare(sKey(b));if(sc!==0)return sc;const ka=_bNo(a.deliveryNo)||"",kb=_bNo(b.deliveryNo)||"";if(ka!==kb)return ka.localeCompare(kb);const sa=a.startDate||"",sb=b.startDate||"";if(sa!==sb)return sa.localeCompare(sb);return(a.isExtension?1:0)-(b.isExtension?1:0);});const _lastMIdx=_sorted.reduce((acc,r,i)=>r.billingType==="monthly"?i:acc,-1);const _hasBoth=_lastMIdx>=0&&_sorted.some(r=>r.billingType!=="monthly");buildChainBlocks(_sorted).forEach((block, _bi) => {
  if (block.type === "chain") {
    ...
    g.items.forEach((r, idx) => {
      ...
      { // ページ分割スコープ
        ...
        const headRestC = (pageNo, totalPagesC) => `<div style="position:relative;margin-bottom:14px">
