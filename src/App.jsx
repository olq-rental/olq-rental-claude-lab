    ...
    <div style={{fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif",minHeight:"100vh",background:"#f1f5f9",color:"#1e293b"}}>
      ...
      <header className="app-header" style={{background:"#0f172a",color:"#fff",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 16px rgba(0,0,0,.4)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontWeight:800,fontSize:15,letterSpacing:2}}>オルク レンタル伝票管理</span><span style={{fontSize:10,color:"#94a3b8",marginLeft:8,fontWeight:400}}>Ver.1.35</span>
        </div>
...
.empty td{height:18px}.biko{font-weight:bold;letter-spacing:6px;vertical-align:top;width:50px}`;
  ...
  if (type === "invoice") {
    ...
    (()=>{const _sorted=[...g.items].sort((a,b)=>{const aM=a.billingType==="monthly"?0:1;const bM=b.billingType==="monthly"?0:1;if(aM!==bM)return aM-bM;const aBase=(a.isExtension?a.extendedFromNo:a.deliveryNo)||"";const bBase=(b.isExtension?b.extendedFromNo:b.deliveryNo)||"";if(aBase!==bBase)return aBase.localeCompare(bBase,"ja");const aIsExt=a.isExtension?1:0;const bIsExt=b.isExtension?1:0;if(aIsExt!==bIsExt)return aIsExt-bIsExt;return(a.endDate||"").localeCompare(b.endDate||"");});const _lastMIdx=_sorted.reduce((acc,r,i)=>r.billingType==="monthly"?i:acc,-1);const _hasBoth=_lastMIdx>=0&&_sorted.some(r=>r.billingType!=="monthly");_sorted.forEach((r,_ri) => {
                                  ...
                                  <td colSpan={8} style={{padding:0,borderBottom:"1px solid #e2e8f0"}}>
                                    <div style={{background:"#f8fafc",padding:"12px 16px 12px 48px"}}>
                                      ...
                                      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,marginBottom:8}}>
                                        ...
                                        <tbody>
                                          {(()=>{const sorted=[...g.items].sort((a,b)=>{const aM=a.billingType==="monthly"?0:1;const bM=b.billingType==="monthly"?0:1;if(aM!==bM)return aM-bM;const aBase=(a.isExtension?a.extendedFromNo:a.deliveryNo)||"";const bBase=(b.isExtension?b.extendedFromNo:b.deliveryNo)||"";if(aBase!==bBase)return aBase.localeCompare(bBase,"ja");const aIsExt=a.isExtension?1:0;const bIsExt=b.isExtension?1:0;if(aIsExt!==bIsExt)return aIsExt-bIsExt;return(a.endDate||"").localeCompare(b.endDate||"");});const lastMIdx=sorted.reduce((acc,r,i)=>r.billingType==="monthly"?i:acc,-1);const hasBoth=lastMIdx>=0&&sorted.some(r=>r.billingType!=="monthly");return sorted.map((r,i)=>(<React.Fragment key={r.id}><tr style={{borderBottom:"1px solid #f1f5f9"}}>
