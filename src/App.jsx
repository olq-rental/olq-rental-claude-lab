    ...
    <div style={{fontFamily:"'Noto Sans JP','Hiragino Sans',sans-serif",minHeight:"100vh",background:"#f1f5f9",color:"#1e293b"}}>
      ...
      <header className="app-header" style={{background:"#0f172a",color:"#fff",padding:"0 20px",height:52,display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,zIndex:100,boxShadow:"0 2px 16px rgba(0,0,0,.4)"}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontWeight:800,fontSize:15,letterSpacing:2}}>オルク レンタル伝票管理</span><span style={{fontSize:10,color:"#94a3b8",marginLeft:8,fontWeight:400}}>Ver.1.32</span>
        </div>
          ...
          <tbody>
            {(()=>{const _s=[...g.items].sort((a,b)=>{const aM=a.billingType==="monthly"?0:1,bM=b.billingType==="monthly"?0:1;if(aM!==bM)return aM-bM;return(a.endDate||"").localeCompare(b.endDate||"");});const _lm=_s.reduce((acc,r,i)=>r.billingType==="monthly"?i:acc,-1);const _hb=_lm>=0&&_s.some(r=>r.billingType!=="monthly");return _s.flatMap((r,_ri)=>{
              ...
              const rows=rLines.map((ln,li)=>{
                ...
                const lineAmt=(showDiscountLine&&r.billingType!=="monthly")?Math.round(listPrice*(ln.quantity||1)*useDaysForLine):Math.round((ln.unitPrice||0)*(ln.quantity||1)*useDaysForLine);
                return(
